...
`plugins.lua` is evaluated by the embedded LuaJIT runtime defined in
[configuration.md](configuration.md), but that does not make it an arbitrary
program. Its source first passes the literal-only syntax check below. This is
the intentional difference from executable `config.lua` and is required
because oll rewrites plugin declarations.
