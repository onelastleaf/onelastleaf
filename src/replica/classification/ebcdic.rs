pub(super) fn looks_like_ebcdic(bytes: &[u8]) -> bool {
    if bytes.len() < 4 {
        return false;
    }
...
pub(super) fn decode_utf_ebcdic(bytes: &[u8]) -> Option<(String, bool)> {
    let i8_bytes = bytes
        .iter()
...
pub(super) fn encode_utf_ebcdic(text: &str) -> Vec<u8> {
...
pub(super) fn decode_ibm037(bytes: &[u8]) -> String {
    bytes
        .iter()
...
pub(super) fn encode_ibm037(text: &str) -> Option<Vec<u8>> {
    text.chars()
        .map(|character| {
            let scalar = u32::from(character);
